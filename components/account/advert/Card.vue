  ...
  <div>
    <div class="advert" v-if="isMobile == false">
      <div class="advert__badge" v-if="false">
        <svg class="advert__safe-badge" width="20" height="24" viewBox="0 0 20 24" fill="none">
          <path
            ...
            fill="#69CE91"></path>
          <path d="M7 12L9 14L13 10" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
          </path>
        </svg>
        <svg class="advert__hamcar-badge" width="24" height="72" viewBox="0 0 24 72" fill="none">
      ...
      <nuxt-link :to="link" class="advert__banner">
        ...
        <span class="advert__flag advert__flag--place" v-if="advert.status == 'published'">
          {{ advert.visit }} بازدید
        </span>
      ...
      <div class="advert__body">
        ...
        <span class="advert__date-wide" v-if="expireDate && advert.status == 'published'">
          {{ RemainingTime(new Date(expireDate)) }}
          روز تا انقضای آگهی
        </span>
        <span class="advert__date-wide mr-1">
          {{ getAdvertStatus() }}
        </span>
        <span @click="useNardeban" class="advert__date-wide nardeban mr-1"
          v-if="advert.status == 'published'">نردبان</span>
        <div class="advert__info mt-1_5">
          <span>مدل {{ advert.year }}</span>
          <span>{{ splitNumber(advert.mileage) }} کیلومتر</span>
        ...
        <div class="advert__footer mt-2">
          <template v-if="showPrice">
            <div class="advert__wrapper" v-if="
              advert.price.advertisementPaymentType ==
              AdvertisementPaymentType.قسطی
            ">
              <p class="advert__price">
                {{ splitNumber(advert.price.ghest?.pishPardakht??"0") }}
                <span>پیش</span>
              </p>
              <p class="advert__price">
                {{ splitNumber(advert.price.ghest?.amountPricePerGhest??"0") }}
