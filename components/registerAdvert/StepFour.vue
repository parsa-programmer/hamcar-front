<template>
  <div class="mt-7">
    <div v-if="isLogin == false">
      <Transition enter-active-class="animate__lightSpeedInRight" leave-active-class="animate__lightSpeedOutLeft">
        <register-advert-step-four-login v-if="loginStep == 1" @toggleLoginStep="changeAuthSteps" />
      </Transition>
      <Transition enter-active-class="animate__lightSpeedInRight" leave-active-class="animate__lightSpeedOutLeft">
        <register-advert-step-four-validate-code @toggleLoginStep="changeAuthSteps" v-if="loginStep == 2"
          @successed="registerSuccess" />
      </Transition>
    </div>
    <div v-else>
      <div class="row">
        <h-select-box placeholder="انتخاب استان" name="province" v-model="userAddress.Province" :data="provinces"
          :disabled="provinces.length == 0" @selectedItem="provinceChnaged" />
        <h-select-box placeholder="انتخاب شهر" name="city" :disabled="cities.length == 0 || userAddress.Province == ''"
          v-model="userAddress.City" :data="cities" />
      </div>
      <h-input name="postalAddress" class="mt-1" v-model="userAddress.PostalAddress"
