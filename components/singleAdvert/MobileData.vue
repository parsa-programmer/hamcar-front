<template>
  <div class="ads-mobile">
    <div class="ads-mobile__slider">
      ...
      <div
        v-if="advert.images.length > 1"
        class="slider__wraper"
        ref="slider"
      >
        <h-slider
          :items="advert.images"
          :item-container-class="'w-full grow-1'"
          :arrows="false"
          row-class="gap-4px"
          draggable-off
          :middle="middel"
          @getCurrentIndex="(val) => (currentIndex = val)"
        >
          <template #custom_arrows="{ nextSlide, prevSlide }">
            <div
              class="navigation__arrow rotate"
              @click="prevSlide"
              v-if="currentIndex > 0"
            >
