<template>
    <div class="profile__content">
      <client-only>
        ...
        <account-top-bar :showAdvertCount="true" :isExhibition="true"/>
        <div class="prof_row">
          ...
          <div class="profile__card mobile-card">
            ...
            <div class="card__desc" v-if="accountStore.exhibition">
              <template v-if="accountStore.exhibition?.plan">
                <h3 class="profile-card__title mt-1">پکیج ها</h3>
  
                <div class="package__info mb-1_5">
                  <span class="profile-card__value"
                    >(پکیج کلاس {{ accountStore.exhibition.plan.planId }})</span
                  >
                  <span class="badge profile-card__value font-6">
                    اعتبار تا
                    {{ RemainingTime(new Date(accountStore.exhibition.plan.expireDate)) }}
                    روز دیگر
                  </span>
                  <span class="profile-card__value font-6">
                    {{ toPersianDate(new Date(accountStore.exhibition.plan.expireDate)) }}
                  </span>
                </div>
              </template>
              <template v-else>
